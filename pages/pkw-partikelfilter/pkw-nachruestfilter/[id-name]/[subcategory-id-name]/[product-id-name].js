           ...
           //     if (!subcategoryRes.ok) throw new Error(`Failed to fetch subcategories for category ID ${category.category_id}: ${subcategoryRes.status}`);
                ...
                //        if (!productRes.ok) throw new Error(`Failed to fetch products for subcategory ${subcategory.category_id}: ${productRes.status}`);
                        ...
                        products.forEach((product) => {
                            const productAlias = product.product_alias ? createSlug(product.product_alias) : createSlug(product.product_name);
                            paths.push({
                                params: {
                                    ...
                                    "product-id-name": `${product.product_id}-${productAlias}`,
                                },
                            });
                        });
                    } catch (error) {
        ...
        <>
            {product && (
                <div className={"row g-0"}>
                    <h1 className={"pb-0 mb-0"}>{product.product_alias ?? product.product_name} für {product.modell_liste}</h1>
                </div>
            )}
