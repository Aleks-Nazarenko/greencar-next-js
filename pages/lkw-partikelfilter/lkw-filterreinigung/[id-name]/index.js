...
export async function getStaticPaths() {
    ...
    try {
        const res = await axios.get(`${JOOMLA_API_BASE}&task=getSubcategories&category_id=470&format=json`);
...
function LkwFilterreinigungProductListPage({ categories, categoryId, categoryName, article, products }) {
            ...
            <div className="row g-0 p-3 p-sm-4 product-detail-view rounded-4 align-items-center">
                ...
                <div className={"col"}>
                    <div className="col text-end d-none d-sm-block">
                        <NextImage src={"/images/icons/lkw-filterreinigung.png"} alt={"LKW Filterreinigung"} width={190} height={190} className={"img-fluid picto-190"}/>
                    </div>
                </div>
                        ...
                        <ul className={"arrow-list"}>
                            {categories.map(category => (
                                <li key={category.category_id}>
                                    ...
                                    <Link href={`/lkw-partikelfilter/lkw-filterreinigung/${category.category_id}-${category.category_name.toLowerCase().replace(/\s+/g, '-')}`}>{category.category_name}</Link>
                                </li>
                            ))}
                        </ul>
                    )}
...
export default LkwFilterreinigungProductListPage;
