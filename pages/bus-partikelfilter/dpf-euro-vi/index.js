...
export async function getStaticProps() {
    ...
    const categories = await axios
        .get(`${JOOMLA_API_BASE}&task=getSubcategories&category_id=69&format=json`)
    ...
    const resArticle = await fetch(`${JOOMLA_API_BASE}&task=articleWithModules&id=21&format=json`);
...
export default function dpfEuroVi({ categories, article }) {
    ...
    const handleCategoryChange = (event) => {
        ...
        if (selectedCategory) {
            router.push(`/bus-partikelfilter/dpf-euro-vi/${selectedCategory}`);
        }
    };
    return (
        <>
            ...
            <div className={"row g-0 pb-0"}>
                <h1 className={"mb-1"}>BUS - Nachrüstfilter</h1>
                <h2 className={"display-4 mb-0"}>Bitte wählen Sie den Hersteller Ihres Bus. Anschließend stellen wir Ihnen unsere Produktauswahl an passenden Bus-Nachrüstfiltern vor.</h2>
            </div>
            ...
            <div className="row g-0 p-3 p-sm-4 product-detail-view rounded-4">
                ...
                <div className="col text-end d-none d-sm-block">
                    <Image src={"/images/icons/bus-nachruestfilter.png"} alt={"Bus Nachrüstfilter"} width={190} height={190} className={"img-fluid picto-190"}/>
                </div>
            </div>
                        ...
                        <ul className={"arrow-list"}>
                            {categories.map(category => (
                                <li key={category.category_id}>
                                    ...
                                    <Link href={`/bus-partikelfilter/dpf-euro-vi/${category.category_id}-${category.category_name.toLowerCase().replace(/\s+/g, '-')}`}>{category.category_name}</Link>
                                </li>
                            ))}
                        </ul>
                    )}
