...
export default function AnfrageFilterreinigungBau() {
    ...
    const handleCheckboxChange = () => {
        setDiffShipping((prevData) => {
            const newState = !prevData;
            if (!newState) {
                // Clear the textarea when the checkbox is unchecked
                setFormData((prevData) => ({ ...prevData, abweichend: "" }));
            }
            return newState;
        });
    };
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const payload = {
                ...
                abweichend: formData.abweichend,

            };
            const response = await fetch(`${JOOMLA_API_BASE}&task=anfrageFilterreinigungBau&format=json`, {
                method: 'POST',
    ...
    return (
        <>
            ...
            <Form noValidate validated={validated} onSubmit={handleSubmit}>
                ...
                <Row className="mb-3 g-0">
                    <Form.Group as={Col} sm="6" controlId="regAbholung">
                        <Form.Label>Abholung gewünscht?</Form.Label>
                        <Form.Select
                            name="abholung"
                            value={formData.abholung}
                            onChange={handleInputChange}
                        >
                            <option value="ja">ja</option>
                            <option value="nein">nein</option>
                ...
                <Row className="mb-3 g-0">
                    <Form.Group as={Col} sm={"6"} controlId="regAbholdatum">
                        <Form.Label>gewünschtes Abholdatum</Form.Label>
                        <Form.Control
                            type="text"
                            name="abholdatum"
                ...
                <Row className="mb-3 g-0">
                    <Form.Group as={Col} sm={"6"} controlId="regAbholzeit">
                        <Form.Label>gewünschte Abholzeit</Form.Label>
                        <Form.Control
                            type="text"
                            name="abholzeit"
                ...
                <Row className="mb-3 g-0">
                    <Form.Group className="">
                        <Form.Check
                            type="checkbox"
                            label="Abweichende Abhol- und Lieferadresse?"
                ...
                {diffShipping && (
                    <Row className="g-0 mb-3">
                        <Form.Group as={Col}  sm={"6"} controlId="regAbweichend">
                            <Form.Control as="textarea" rows={5}
                                          name="abweichend"
                                          value={formData.abweichend ?? ""}
                                          onChange={handleInputChange}
                            />
                        </Form.Group>
                    </Row>
